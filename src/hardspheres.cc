...
void if_bead_coll_sol(const std::vector<Vec3> &pos,
                      ...
                      const std::vector<Vec3> &pos_sol,
                      const std::vector<Vec3> &vel_sol, const double rh2,
...
void if_sol_coll_sol(const std::vector<Vec3> &pos_sol,
                     const std::vector<Vec3> &vel_sol, const double rh2,
                     const Box &box, const std::vector<uint64_t> &counter_sol,
                     EventQueue &event_queue,
                     const std::vector<double> &times_sol, unsigned int i,
                     unsigned int j) {
  double t, dx, dy, dz, dvx, dvy, dvz;
  delta_tpv(pos_sol, vel_sol, pos_sol, vel_sol, box, times_sol, times_sol, i, j,
            t, dx, dy, dz, dvx, dvy, dvz);
...
void iterate_bead_coll_sol(
    ...
    const double rh2, const Box &box, const std::vector<uint64_t> &counter,
    ...
    const Cells &cells, unsigned int icell, unsigned int i) {
  for (unsigned int i_sol : cells.cells[icell]) {
    if_bead_coll_sol(pos, vel, pos_sol, vel_sol, rh2, box, counter, counter_sol,
                     event_queue, times, times_sol, i, i_sol);
  }
}
...
void iterate_sol_coll_bead(
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
    const double rh2, const Box &box, const std::vector<uint64_t> &counter_sol,
    const std::vector<uint64_t> &counter, EventQueue &event_queue,
...
void iterate_coll_sol(const std::vector<Vec3> &pos_sol,
                      const std::vector<Vec3> &vel_sol, const double rh2,
                      const Box &box, const std::vector<uint64_t> &counter_sol,
                      EventQueue &event_queue,
                      const std::vector<double> &times_sol, const Cells &cells,
                      unsigned int icell, unsigned int i) {
  for (unsigned int j : cells.cells[icell]) {
    if_sol_coll_sol(pos_sol, vel_sol, rh2, box, counter_sol, event_queue,
                    times_sol, i, j);
  }
}
  ...
  for (unsigned int z = zmin; z <= zmax; z++) {
    for (unsigned int y = ymin; y <= ymax; y++) {
      for (unsigned int x = xmin; x <= xmax; x++) {
        ...
        iterate_bead_coll_sol(pos, vel, pos_sol, vel_sol, rh2, box, counter,
                              counter_sol, event_queue, times, times_sol,
                              cells_sol, icell, i);
      }
    }
  }
  ...
  for (unsigned int z = zmin; z <= zmax; z++) {
    for (unsigned int y = ymin; y <= ymax; y++) {
      for (unsigned int x = xmin; x <= xmax; x++) {
        ...
        iterate_sol_coll_bead(pos_sol, vel_sol, pos, vel, rh2, box, counter_sol,
                              counter, event_queue, times_sol, times,
                              cells_bead, icell, i_sol);
      }
    }
  }
}

void add_events_for_one_sol(const std::vector<Vec3> &pos_sol,
                            const std::vector<Vec3> &vel_sol, const double rh2,
                            const Box &box,
                            const std::vector<uint64_t> &counter_sol,
                            EventQueue &event_queue,
                            const std::vector<double> &times_sol,
                            const Cells &cells_sol, unsigned int i_sol) {
  unsigned int zmin, zmax, ymin, ymax, xmin, xmax;
  walls_of_cell(zmin, zmax, ymin, ymax, xmin, xmax, cells_sol, i_sol);
  ...
  for (unsigned int z = zmin; z <= zmax; z++) {
    for (unsigned int y = ymin; y <= ymax; y++) {
      for (unsigned int x = xmin; x <= xmax; x++) {
        ...
        unsigned int icell = icell_x + icell_y * cells_sol.ncell +
                             icell_z * cells_sol.ncell * cells_sol.ncell;
        iterate_coll_sol(pos_sol, vel_sol, rh2, box, counter_sol, event_queue,
                         times_sol, cells_sol, icell, i_sol);
      }
    }
  }
}
...
void add_events_for_all_bead_coll_sol(
    const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    unsigned int nbeads, const double rh2, const Box &box,
    const std::vector<uint64_t> &counter,
    const std::vector<uint64_t> &counter_sol, EventQueue &event_queue,
    const std::vector<double> &times, const std::vector<double> &times_sol,
    const Cells &cells_bead, const Cells &cells_sol) {
  assert(pos.size() == nbeads);
  for (unsigned int i = 0; i < nbeads; i++) {
    add_events_for_one_bead_coll_sol(pos, vel, pos_sol, vel_sol, rh2, box,
                                     counter, counter_sol, event_queue, times,
                                     times_sol, cells_bead, cells_sol, i);
  }
}

void add_events_for_all_sol_coll_bead(
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    const std::vector<Vec3> &pos, const std::vector<Vec3> &vel,
    unsigned int nsol, const double rh2, const Box &box,
    const std::vector<uint64_t> &counter_sol,
    const std::vector<uint64_t> &counter, EventQueue &event_queue,
    const std::vector<double> &times_sol, const std::vector<double> &times,
    const Cells &cells_bead, const Cells &cells_sol) {
  ...
  for (unsigned int i = 0; i < nsol; i++) {
    add_events_for_one_sol_coll_bead(
        pos_sol, vel_sol, pos, vel, rh2, box, counter_sol, counter, event_queue,
        times_sol, times, cells_bead, cells_sol, i);
  }
}

void add_events_for_all_sol(const std::vector<Vec3> &pos_sol,
                            const std::vector<Vec3> &vel_sol, unsigned int nsol,
                            const double rh2, const Box &box,
                            const std::vector<uint64_t> &counter_sol,
                            EventQueue &event_queue,
                            const std::vector<double> &times_sol,
                            const Cells &cells_sol) {
  assert(pos_sol.size() == nsol);
  for (unsigned int i = 0; i < nsol; i++) {
    add_events_for_one_sol(pos_sol, vel_sol, rh2, box, counter_sol, event_queue,
                           times_sol, cells_sol, i);
  }
}
...
void add_events_for_sol_after_crossing(
    const std::vector<Vec3> &pos_sol, const std::vector<Vec3> &vel_sol,
    const double rh2, const Box &box, const std::vector<uint64_t> &counter_sol,
    EventQueue &event_queue, const std::vector<double> &times_sol,
    const Cells &cells_sol, unsigned int i_sol, SolventCellEvent::Wall wall) {
  unsigned int zmin, zmax, ymin, ymax, xmin, xmax;
  walls_of_cell(zmin, zmax, ymin, ymax, xmin, xmax, cells_sol, i_sol);
...
bool check_overlap(const unsigned int i, const std::vector<Vec3> &pos,
                   ...
                   const std::vector<Vec3> &pos_sol,
                   const std::vector<double> &times,
                   const std::vector<double> &times_sol, const double rh2,
                   const Box &box) {
  const unsigned int nbeads = pos.size();
...
bool process_event(const MinNearestEvent &ev, System &sys, const Param &p,
  ...
  add_events_for_one_bead(sys.pos, sys.vel, p.rh2, p.rc2, box, sys.counter,
                          ...
                          p.transient_bonds, p.permanent_bonds, update_config,
                          p.max_nbonds);
  add_events_for_one_bead_coll_sol(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol,
                                   p.sigma_sb2, box, sys.counter,
                                   sys.counter_sol, event_queue, sys.times,
                                   sys.times_sol, cells_bead, cells_sol, ev.i);
  add_events_for_one_bead_coll_sol(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol,
                                   p.sigma_sb2, box, sys.counter,
                                   sys.counter_sol, event_queue, sys.times,
                                   sys.times_sol, cells_bead, cells_sol, ev.j);
...
bool process_event(const BeadCollidesSolEvent &ev, System &sys, const Param &p,
  ...
  add_events_for_one_bead(sys.pos, sys.vel, p.rh2, p.rc2, box, sys.counter,
                          ...
                          p.transient_bonds, p.permanent_bonds, update_config,
                          p.max_nbonds);
  add_events_for_one_sol(sys.pos_sol, sys.vel_sol, p.sigma_ss2, box,
                         sys.counter_sol, event_queue, sys.times_sol, cells_sol,
                         ev.j);
  add_events_for_one_bead_coll_sol(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol,
                                   p.sigma_sb2, box, sys.counter,
                                   sys.counter_sol, event_queue, sys.times,
                                   sys.times_sol, cells_bead, cells_sol, ev.i);
  add_events_for_one_sol_coll_bead(sys.pos_sol, sys.vel_sol, sys.pos, sys.vel,
                                   p.sigma_sb2, box, sys.counter_sol,
                                   sys.counter, event_queue, sys.times_sol,
                                   sys.times, cells_bead, cells_sol, ev.j);

  if_cell(sys.pos, sys.vel, box, sys.counter, event_queue, sys.times, ev.i,
          cells_bead);
  if_cell_sol(sys.pos_sol, sys.vel_sol, box, sys.counter_sol, event_queue,
              sys.times_sol, ev.j, cells_sol);

  return true;
}

bool process_event(const SolCollidesSolEvent &ev, System &sys, const Param &p,
                   const Box &box, EventQueue &event_queue, Cells &cells_bead,
  ...
  update_pos(sys.pos_sol[ev.j], sys.vel_sol[ev.j], sys.times_sol[ev.j], ev.t);
  assert(check_overlap(ev.i, sys.pos_sol, sys.vel_sol, sys.pos_sol,
                       sys.times_sol, sys.times_sol, p.sigma_ss2, box));
  assert(check_overlap(ev.j, sys.pos_sol, sys.vel_sol, sys.pos_sol,
                       sys.times_sol, sys.times_sol, p.sigma_ss2, box));

  final_vel(sys.pos_sol, sys.vel_sol, sys.pos_sol, sys.vel_sol, p.sigma_ss2,
            ev.i, ev.j, box, {}, p.m_sol, p.m_sol);
  ...
  add_events_for_one_sol_coll_bead(sys.pos_sol, sys.vel_sol, sys.pos, sys.vel,
                                   p.sigma_sb2, box, sys.counter_sol,
                                   sys.counter, event_queue, sys.times_sol,
                                   sys.times, cells_bead, cells_sol, ev.i);
  add_events_for_one_sol_coll_bead(sys.pos_sol, sys.vel_sol, sys.pos, sys.vel,
                                   p.sigma_sb2, box, sys.counter_sol,
                                   sys.counter, event_queue, sys.times_sol,
                                   sys.times, cells_bead, cells_sol, ev.j);
...
bool process_event(const SolventCellEvent &ev, System &sys, const Param &p,
  ...
  add_events_for_sol_after_crossing(sys.pos_sol, sys.vel_sol, p.sigma_ss2, box,
  ...
  add_events_for_sol_coll_bead_after_crossing(
      ...
      cells_bead, cells_sol, ev.i, ev.wall);
