...
void initialize_system(System &sys, Random &mt, const Param &p, const Box &box,
    ...
    if (!(cells_sol.lcell >= p.sigma_ss)) {
        throw std::invalid_argument("sol lcell must be at least sigma_ss");
    }

    init_vel(sys.vel, mt, p.temp, p.m);

    init_vel_sol(sys.vel_sol, p.nsol, mt);
    ...
    init_cell_sol_events(sys.pos_sol, sys.vel_sol, p.nsol, box, sys.counter_sol, event_queue, sys.times_sol, cells_sol);
    ...
    add_events_for_all_bead_coll_sol(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol, p.nbeads, p.sigma_sb2, box, sys.counter, sys.counter_sol, event_queue, sys.times, sys.times_sol, cells_bead, cells_sol);
    add_events_for_all_sol_coll_bead(sys.pos_sol, sys.vel_sol, sys.pos, sys.vel, p.nsol, p.sigma_sb2, box, sys.counter_sol, sys.counter, event_queue, sys.times_sol, sys.times, cells_bead, cells_sol);
    add_events_for_all_sol(sys.pos_sol, sys.vel_sol, p.nsol, p.sigma_ss2, box, sys.counter_sol, event_queue, sys.times_sol, cells_sol);
}
...
void run_step(System &sys, const Param &p, const Box &box, UpdateConfig
    ...
    while (!event_queue.empty()) {
        ...
        if (std::visit([&](auto&& ev) {
                    ...
                    return process_event(ev, sys, p, box, event_queue, cells_bead, cells_sol, update_config, update_config_writer);
                    }, event)) validevents++;
    ...
    for (unsigned int i = 0; i < p.nsol; i++) {
        ...
        // check overlap between beads and solvent particles
        assert(check_overlap(i, sys.pos, sys.vel, sys.pos_sol, sys.times, sys.times_sol, p.sigma_sb2, box));
        // check overlap between solvent particles
        assert(check_overlap(i, sys.pos_sol, sys.vel_sol, sys.pos_sol, sys.times_sol, sys.times_sol, p.sigma_ss2, box));
    }
...
void run_trajectory_eq(System &sys, const Param &p, const Box &box, UpdateConfig &update_config, UpdateConfigWriter &update_config_writer, Cells& cells_bead, Cells &cells_sol, EventQueue &event_queue, uint64_t countevents, uint64_t validevents, double wall_time) {
...
void run_trajectory(System &sys, const Param &p, const Box &box, UpdateConfig &update_config, UpdateConfigWriter &update_config_writer, Cells& cells_bead, Cells &cells_sol, EventQueue &event_queue, PosWriter &pos_writer, VelWriter &vel_writer, ConfigWriter &config_writer, std::set<Config> &store_config, ConfigInt &store_config_int, std::vector<uint64_t> &config_sum, uint64_t countevents, uint64_t validevents, double wall_time, unsigned int iter, const H5::DataSpace &mem_space, H5::DataSpace &file_space, H5::DataSet &dataset_pos) {
    for (unsigned int step = 0; step < p.nsteps; step++) {
        std::cout << "step = " << step << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    for (unsigned int iter = 0; iter < p.total_iter; iter++) {
        ...
        run_trajectory_eq(sys, p_eq, box, update_config_eq, update_config_writer, cells_bead, cells_sol, event_queue_eq, countevents, validevents, wall_time);
        ...
        run_trajectory(sys, p, box, update_config, update_config_writer, cells_bead, cells_sol, event_queue, pos_writer, vel_writer, config_writer, store_config, store_config_int, config_sum, countevents, validevents, wall_time, iter, mem_space, file_space, dataset_pos);
...
void from_json(const nlohmann::json& json, Param& p) {
    ...
    p.sigma_ss = json["sigma_ss"];
    p.sigma_ss2 = p.sigma_ss * p.sigma_ss;
