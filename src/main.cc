...
void initialize_system(System &sys, Random &mt, const Param &p, const Box &box,
  ...
  init_cell_sol_events(sys.pos_sol, sys.vel_sol, p.nsol, box, sys.counter_sol,
                       event_queue, sys.times_sol, cells_sol);
  ...
  add_events_for_all_beads(sys.pos, sys.vel, p.nbeads, p.rh2, p.rc2, box,
                           ...
                           p.max_nbonds);
  add_events_for_all_bead_coll_sol(sys.pos, sys.vel, sys.pos_sol, sys.vel_sol,
                                   p.nbeads, p.sigma_sb2, box, sys.counter,
                                   sys.counter_sol, event_queue, sys.times,
                                   sys.times_sol, cells_bead, cells_sol);
  add_events_for_all_sol_coll_bead(sys.pos_sol, sys.vel_sol, sys.pos, sys.vel,
                                   p.nsol, p.sigma_sb2, box, sys.counter_sol,
                                   sys.counter, event_queue, sys.times_sol,
                                   sys.times, cells_bead, cells_sol);
  add_events_for_all_sol(sys.pos_sol, sys.vel_sol, p.nsol, p.sigma_ss2, box,
...
void run_step(System &sys, const Param &p, const Box &box,
  ...
  for (unsigned int i = 0; i < p.nsol; i++) {
    ...
    assert(check_overlap(i, sys.pos_sol, sys.vel_sol, sys.pos_sol,
...
void run_trajectory_eq(System &sys, const Param &p, const Box &box,
                       ...
                       Cells &cells_bead, Cells &cells_sol,
                       EventQueue &event_queue, uint64_t countevents,
                       uint64_t validevents, double wall_time) {
...
void run_trajectory(System &sys, const Param &p, const Box &box,
                    ...
                    ConfigWriter &config_writer, std::set<Config> &store_config,
                    ConfigInt &store_config_int,
                    std::vector<uint64_t> &config_sum, uint64_t countevents,
                    uint64_t validevents, double wall_time, unsigned int iter,
                    const H5::DataSpace &mem_space, H5::DataSpace &file_space,
...
int main(int argc, char *argv[]) {
  ...
  for (unsigned int iter = 0; iter < p.total_iter; iter++) {
    ...
    run_trajectory_eq(sys, p_eq, box, update_config_eq, update_config_writer,
                      cells_bead, cells_sol, event_queue_eq, countevents,
                      validevents, wall_time);
    ...
    run_trajectory(sys, p, box, update_config, update_config_writer, cells_bead,
                   cells_sol, event_queue, pos_writer, vel_writer,
                   config_writer, store_config, store_config_int, config_sum,
                   countevents, validevents, wall_time, iter, mem_space,
